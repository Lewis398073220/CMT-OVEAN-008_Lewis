...
A2DP_VIRTUAL_SURROUND ?= 0

###A2DP_VIRTUAL_SURROUND_STEREO=1  ##Disable by Jay. if enable will be have noise at 3.5 line in mode.
...
## PC_CMD_UART ?= 1 #ENABLE BY JAY, for debug A2DP EQ. Normal mode must need to disable, otherwise debug_log exception
export PC_CMD_UART ?= 0
ifeq ($(PC_CMD_UART),1)
KBUILD_CPPFLAGS += -DPC_CMD_UART
endif
...
SPEECH_TX_AEC2FLOAT ?= 1 #ENABLE BY JAY
...
SPEECH_TX_3MIC_NS ?= 0
SPEECH_RX_COMPEXP ?= 1 #ENABLE BY JAY

#SPEECH_RX_EQ ?= 1 #ENABLE BY JAY
# Add BY JAY
...
#SPEECH_RX_POST_GAIN ?= 1 #ENABLE BY JAY
# Add BY JAY
