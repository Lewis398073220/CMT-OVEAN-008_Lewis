...
export CHIP_HAS_SECURE_BOOT := 1
ifeq ($(ROM_VERSION_V2),1)
KBUILD_CPPFLAGS += -DROM_VERSION_V2
...
export BLE_AUDIO_USB_DIR_PATH = bthost/service/ble_audio/ble_usb_audio
...
export BT_HOST_REJECT_UNEXCEPT_SCO_PACKET ?= 0
...
export USE_PAGE_SCAN_REPETITION_MODE_R1 ?= 1
...
KBUILD_CPPFLAGS += -DSPEECH_ALGO_DSP_HIFI
export APP_RPC_BTH_DSP_EN := 1
else ifeq ($(AUDIO_DSP_ACCEL),SENS)
KBUILD_CPPFLAGS += -DSPEECH_ALGO_DSP_SENS
...
ifeq ($(SPEECH_ALGO_DSP),M55)
KBUILD_CPPFLAGS += -DSPEECH_ALGO_DSP_M55
...
ifeq ($(USE_APP_AUDIO_THREAD),1)
KBUILD_CPPFLAGS += -DUSE_APP_AUDIO_THREAD
endif
...
KBUILD_CPPFLAGS += -DBLE_ADV_REGENERATE_NEW_RPA_DURATION=15*60
endif
...
ifneq ($(filter 1, $(BLE_AUDIO_ENABLED) $(KEEP_BLE_AUDIO_IN_NV_RECORD)),)
export NEW_IMAGE_FLASH_OFFSET := 0x260000
else
export NEW_IMAGE_FLASH_OFFSET := 0x240000
endif
KBUILD_CPPFLAGS += -DNEW_IMAGE_FLASH_OFFSET=$(NEW_IMAGE_FLASH_OFFSET)
else
ifeq ($(FLASH_SIZE)-$(OTA_BIN_COMPRESSED), 0x200000-1)
...
KBUILD_CPPFLAGS += -DAAC_MEMPOOL_SIZE=32*1024-400
endif
# vvvvvvvvvvvvvvvvvvvvvvvvvvvvv
...
ifeq ($(SPEECH_TX_NS8),1)
export SPEECH_TX_NS8
KBUILD_CPPFLAGS += -DSPEECH_TX_NS8
export NN_LIB ?= 1
export DSP_LIB ?= 1
export BECO ?= 1
endif
...
OTA_APP_BOOT_INFO_SECTION_SIZE ?= 0x1000
else
OTA_APP_BOOT_INFO_SECTION_SIZE ?= 0
endif
