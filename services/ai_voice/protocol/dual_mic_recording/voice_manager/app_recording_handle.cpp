...
typedef struct
{
    uint32_t LatestDmaIrqTime;
    uint32_t LatestCursor;
} RECORDING_INFO_T;
...
static uint8_t flagLocalReady = 0;
static uint8_t flagPeerReady = 0;
static uint8_t flagSppSendDone = 1;
static RECORDING_INFO_T peerRecordInfo = {0, 0};
static RECORDING_INFO_T localRecordInfo = {0, 0};
static bool firstSyncRecordInfo = true;
...
#endif
static void app_ibrt_tws_master_clkcnt_get(uint16_t tws_conhdl, uint32_t btclk, uint16_t btcnt,
...
static void recording_data_cursor_sync(void)
{
    int32_t cursorDiffValue;
    static int32_t totalCursorDiff = 0;
    int32_t framePeriodInUs = 15 * 1000;
#ifdef RECORDING_USE_OPUS
    framePeriodInUs = VOICE_OPUS_FRAME_PERIOD_IN_MS *1000;
#endif

    if(0 != (((int32_t)localRecordInfo.LatestCursor-totalCursorDiff) - (int32_t)peerRecordInfo.LatestCursor)){
        TRACE(0,"[%s]cursor %d %d %d", __func__, localRecordInfo.LatestCursor, peerRecordInfo.LatestCursor, totalCursorDiff);
        return;
    }

    cursorDiffValue = ((int32_t)localRecordInfo.LatestDmaIrqTime - (int32_t)peerRecordInfo.LatestDmaIrqTime) / framePeriodInUs;
    TRACE(0,"[%s] diff (d)%d (u)%u  cursor %d %d", __func__, cursorDiffValue, (uint32_t)abs(cursorDiffValue), curCursor, revCursorPeer);

    uint32_t lock = int_lock();
    if(cursorDiffValue != 0){
        if(cursorDiffValue < 0){
            curCursor = curCursor - (uint32_t)abs(cursorDiffValue);
        }else{
            curCursor = curCursor + (uint32_t)abs(cursorDiffValue);
        }
        totalCursorDiff += cursorDiffValue;
    }

    int_unlock(lock);
    TRACE(0,"[%s] Time %u %u cursor %d %d startdiff %d", __func__, localRecordInfo.LatestDmaIrqTime, peerRecordInfo.LatestDmaIrqTime,\
            localRecordInfo.LatestCursor, peerRecordInfo.LatestCursor, totalCursorDiff);
    TRACE(0,"[%s] cursor %d %d", __func__, curCursor, revCursorPeer);

}

static void app_refresh_local_recording_info()
{
    uint32_t btclk;
    uint16_t btcnt;
    ...
    localRecordInfo.LatestCursor = curCursor;
    ...
    if(adma_ch != HAL_DMA_CHAN_NONE)
    {
            ...
            bt_drv_reg_op_dma_tc_clkcnt_get_by_ch(&btclk, &btcnt, adma_ch & 0xFF);
            app_ibrt_tws_master_clkcnt_get(p_ibrt_ctrl->tws_conhandle, btclk, btcnt,
                                        &tws_master_clk, &tws_master_cnt);
            localRecordInfo.LatestDmaIrqTime = bt_syn_ble_bt_time_to_bts(tws_master_clk, tws_master_cnt);
    }

    if(TWS_UI_MASTER == app_ibrt_if_get_ui_role())  //ui role
    {
        tws_ctrl_send_cmd(APP_IBRT_CUSTOM_CMD_RECORD_INFO, (uint8_t*)&localRecordInfo, sizeof(localRecordInfo));
    }
    TRACE(0, "[%s] time %u cursor %d", __func__, localRecordInfo.LatestDmaIrqTime, localRecordInfo.LatestCursor);
}

void app_recording_info_slave_received(uint16_t rsp_seq, uint8_t* data)
{

    tws_ctrl_send_rsp(APP_IBRT_CUSTOM_CMD_RECORD_INFO, rsp_seq, (uint8_t*)&localRecordInfo, sizeof(localRecordInfo));
    TRACE(0, "[%s] time %u cursor %d", __func__, localRecordInfo.LatestDmaIrqTime, localRecordInfo.LatestCursor);
}

void app_recording_info_master_received(uint8_t* data)
{    
    memcpy(&peerRecordInfo, data, sizeof(peerRecordInfo));
    TRACE(0, "[%s] Stime %u", __func__, peerRecordInfo.LatestDmaIrqTime);

    if(peerRecordInfo.LatestDmaIrqTime){
        recording_data_cursor_sync();
    }
    TRACE(0, "[%s] Stime %u cursor %d", __func__, peerRecordInfo.LatestDmaIrqTime, peerRecordInfo.LatestCursor);
}
...
void VoiceShareThread(void const *argument)
{
    while (1)
    {
        osSignalWait(SIGNAL_VOICE_SHARE, osWaitForever);
shareAgain:
        int status = CQ_OK;
        ...
        if (flagLocalReady)
        {
            TRACE(0,"SIGNAL_LOCAL_DATA_READY flagLocalReady %d",flagLocalReady);
            flagLocalReady--;
            ...
            else if (voice_compression_get_encode_buf_size())
            {
                ...
                if(!(++count%50)){
                    app_refresh_local_recording_info();
                    count = 0;
                }
...
transmit_data_to_master:
        ...
        if (flagSppSendDone && flagPeerReady && \
            TWS_UI_MASTER == app_ibrt_if_get_ui_role())
        {
            TRACE(0,"SIGNAL_SPP_SEND_DONE flagSppSendDone %d",flagSppSendDone);
            flagSppSendDone--;
            flagPeerReady--;
                ...
                (TWS_UI_MASTER == app_ibrt_if_get_ui_role()))
            {
                    ...
                    (revCursorPeer - outCursor) >= CUMULATE_PACKET_THRESHOLD)
                {
                    ...
                    for (uint8_t cumulated_packet = 0; cumulated_packet < CUMULATE_PACKET_THRESHOLD; cumulated_packet++)
                    {
                        ...
                        /*NEW DATA FORMAT*******************************************************************************/
                        /*DATA(Bitrate 64kbps):HEADER(4bytes)+ENCODED DATA(160bytes)************************************/
                        /***********************************************************************************************/
                        ...
                        *pRchnl = plcFlag;
                        ...
                        else if ((localIndex != outCursor) && (peerIndex != outCursor))
                        {
                            ...
                            TRACE(4, "[Recording warning]-2: index:%d %d Cursor: out->%d cur->%d",
                                localIndex, peerIndex, outCursor, curCursor);
                            if(outCursor>localIndex && outCursor>peerIndex){
                                outCursor -= 2;
                                local_free = false;
                                peer_free = false;
                            ...
                            if(localIndex > outCursor){
                                local_free = false;
                                memcpy(pLocal + HEADER_SIZE, encoded_zero, encoded_zero_size);
                                memset(pLocal+ encoded_zero_size + HEADER_SIZE, 0x00, encodedSize - encoded_zero_size);
                            }
                            if(peerIndex > outCursor){
                                peer_free = false;
                                memcpy(pSlave + HEADER_SIZE, encoded_zero, encoded_zero_size);
                                memset(pSlave + encoded_zero_size + HEADER_SIZE, 0x00, encodedSize - encoded_zero_size);
                            }
                        ...
                        {
                            if (app_recording_spp_send_data((uint8_t *)&recording_data, sendLen))
                            {
...
#else
                                TRACE(2, "[Recording warning]%s: spp send busy cumulated_packet=%d", __func__,cumulated_packet);
#endif
                            }
                        }
#else
...
void app_recording_store_peer_data(void *param1, uint32_t param2)
{
    ...
    {
        TRACE(1, "[%s] revCursorPeer:%d", __func__,revCursorPeer);
        // osSignalSet(voiceTid, SIGNAL_PEER_DATA_READY);
        osSignalSet(voiceTid, SIGNAL_VOICE_SHARE);
        flagPeerReady++;
    }
    revCursorPeer++;
    return;
...
#endif
    TRACE(0,"[%s] enable %d",__func__, recording_enable);
...
uint32_t app_recording_connected(void *param1, uint32_t param2, uint8_t dest_id)
{
    ...
    ai_manager_set_current_spec(AI_SPEC_RECORDING);
...
static void reset_env()
{
    ...
    flagLocalReady = 0;
    flagSppSendDone = 1;
    firstSyncRecordInfo = true;
    memset(&peerRecordInfo, 0, sizeof(peerRecordInfo));
    memset(&localRecordInfo, 0, sizeof(localRecordInfo));
    voice_compression_reset_encode_buf();
}
...
uint32_t app_recording_disconnected(void *param1, uint32_t param2)
{
    ...
    else if (transType == AI_TRANSPORT_BLE)
    {
        app_recording_bt_connect_cb(BLE_DISCONNECT);
    }

    app_ai_disconnect_handle(transType, AI_SPEC_RECORDING);
...
{
    TRACE(1, "%s: curCursor %d, outCursor %d recording_enable %d", __func__, curCursor, outCursor, recording_enable);
    flagSppSendDone++;
    osSignalSet(voiceTid, SIGNAL_VOICE_SHARE);
