...
#ifdef OTA_BOOT_USE_FLASH_NC_SECTION
extern uint32_t __ota_app_boot_info_start[];
extern uint32_t __ota_app_boot_info_end[];
static const FLASH_OTA_BOOT_INFO_T otaAppBootInfoInFlash  =
    {NORMAL_BOOT, 0, 0, (OTA_FLASH_LOGIC_ADDR + NEW_IMAGE_FLASH_OFFSET), BOOT_WORD_A};
#endif
...
APP_OTA_BOOT_INFO_FLASH_STRUCT_T boot_info_of_secure_boot_app;
#endif
...
#endif
#if (BT_DEVICE_NUM > 1)
#define IBRT_OTA_BUF_PENDING_MAX (10)
#else
#define IBRT_OTA_BUF_PENDING_MAX (5)
#endif
static uint8_t ota_image_buff_pending_num = 0;
#endif
...
#ifdef SECURE_BOOT_WORKAROUND_SOLUTION
void ota_boot_info_env_setup_for_secure_boot()
{
    boot_info_of_secure_boot_app.flash_boot_info = otaAppBootInfoInFlash;
    boot_info_of_secure_boot_app.single_mode_en = false;
}

void * ota_boot_info_configure_ptr_get(void)
{
    return (void*)&boot_info_of_secure_boot_app;
}

static void ota_boot_info_syc_write_flash_for_secure_boot(void)
{
    ...
    memcpy((uint8_t *)&(boot_info_of_secure_boot_app.flash_boot_info), (uint8_t *)otaBootInfoAddr, sizeof(FLASH_OTA_BOOT_INFO_T));

    app_flash_sector_erase(mod,
                         0);
    app_flash_program(mod,
                      0,
                      (uint8_t *)&boot_info_of_secure_boot_app,
                      sizeof(boot_info_of_secure_boot_app),
                      true);
}

/*
    currently only used to update the instant info except ota itself of APP_OTA_BOOT_INFO_FLASH_STRUCT_T
*/
void ota_boot_info_configure_for_secure_boot(uint16_t offset, uint8_t *buf,uint16_t len)
{
    TRACE(3,"%s [%d] offset %d %d ",__func__,sizeof(APP_OTA_BOOT_INFO_FLASH_STRUCT_T),offset,len);
    if(offset +len> sizeof(APP_OTA_BOOT_INFO_FLASH_STRUCT_T)){
        return;
    }
    uint8_t *boot_info_ptr = (uint8_t*)&boot_info_of_secure_boot_app;
    memcpy(boot_info_ptr+offset,buf,len);
    DUMP8("0x%02x ",boot_info_ptr+offset,len);
}

void ota_boot_info_operation_single_mode_download(uint32_t operation)
{
    uint16_t offset = OFFSETOF(APP_OTA_BOOT_INFO_FLASH_STRUCT_T, single_mode_en);
    
    ota_boot_info_configure_for_secure_boot(offset,(uint8_t*)&operation,sizeof(operation));
    ota_boot_info_syc_write_flash_for_secure_boot();
}
#endif
...
void Bes_exit_ota_state(void)
{
    ...
    tws_ctrl_free_cmd(IBRT_OTA_TWS_IMAGE_BUFF);
#else
...
{
    tws_ctrl_free_cmd(IBRT_OTA_TWS_IMAGE_BUFF);
    tws_ctrl_free_cmd(IBRT_OTA_TWS_SEGMENT_CRC_CMD);
    ota_image_buff_pending_num = 0;
...
#ifdef OTA_BOOT_USE_FLASH_NC_SECTION
uint32_t ota_common_get_boot_info_secion_size(void)
{
    return ((uint32_t)__ota_app_boot_info_end - (uint32_t)__ota_app_boot_info_start);
}
#endif

uint32_t ota_common_get_boot_info_flash_offset(void)
{
#ifndef OTA_BOOT_USE_FLASH_NC_SECTION
...
#else
    TRACE(2,"%s addr = 0x%02x",__func__,(uint32_t)__ota_app_boot_info_start);
    return (uint32_t)__ota_app_boot_info_start;
#endif
}
...
// #endif
        ...
        case OTA_COMMAND_CONFIG_OTA:
        {
            ...
            Bes_enter_ota_state();
            memcpy((uint8_t *)ptConfig, &(otaBuf[1]), dataLenth - 1);
...
#endif
    {
        TRACE(0,"[%s] %d",__func__,ota_image_buff_pending_num);
        ota_image_buff_pending_num++;
        if(ota_image_buff_pending_num > IBRT_OTA_BUF_PENDING_MAX)
        {
            return;
        }
        tws_ctrl_send_cmd(IBRT_OTA_TWS_IMAGE_BUFF, otaBuf, len);
...
void ota_image_buff_pending_num_consume(void)
{
    if(ota_image_buff_pending_num != 0)
    {
        ota_image_buff_pending_num--;
    }
}
...
uint32_t app_get_magic_number(void)
{
    ...
    *(volatile uint32_t *)(magic + 0x400);
    ...
    magic = (volatile uint32_t *)&otaBootInfoAddr;
...
#ifdef OTA_BOOT_USE_FLASH_NC_SECTION
void app_ota_boot_info_section_dump(void)
{
    uint32_t addr = ota_common_get_boot_info_flash_offset();
    uint32_t size = ota_common_get_boot_info_secion_size();
    LOG_D("%s section : start_addr= 0x%02x size = 0x%02x",__func__,addr,size);
    DUMP8("0x%02x ",(uint8_t*)addr,sizeof(FLASH_OTA_BOOT_INFO_T));

    LOG_D("instant:%p ",(uint8_t*)&otaAppBootInfoInFlash);
    DUMP8("0x%02x ",(uint8_t*)&otaAppBootInfoInFlash,sizeof(FLASH_OTA_BOOT_INFO_T));

}

void ota_app_boot_info_rebuild(void)
{
    FLASH_OTA_BOOT_INFO_T otaBootInfo;
    NORFLASH_API_MODULE_ID_T mod = NORFLASH_API_MODULE_ID_BOOTUP_INFO;

    memcpy((uint8_t *)&otaBootInfo, (uint8_t *)&otaAppBootInfoInFlash, sizeof(FLASH_OTA_BOOT_INFO_T));

    app_ota_boot_info_section_dump();
...
#ifdef SECURE_BOOT_WORKAROUND_SOLUTION
    boot_info_of_secure_boot_app.flash_boot_info = otaBootInfo;
    app_flash_program(mod,
                      0,
                      (uint8_t *)&boot_info_of_secure_boot_app,
                      sizeof(boot_info_of_secure_boot_app),
