...
osMutexDef(NORFLASH_API_MUTEX);
static osMutexId norflash_api_mutex_id = NULL;
#endif
...
static int norflash_api_flush_in_task(void);
...
{
#if defined(RTOS) && defined(FLASH1_CTRL_BASE) && defined(USE_MULTI_FLASH)
    norflash_api_mutex_id = osMutexCreate(osMutex(NORFLASH_API_MUTEX));
...
static inline void _norflash_api_mutex_wait(void)
{
    ...
    {
        osMutexWait(norflash_api_mutex_id, osWaitForever);
    }
#endif
}
...
static void _bp_init_2m_128m(enum HAL_FLASH_ID_T id, uint32_t total_size)
{
    ...
    while(1)
    {
        ...
        unprot_size = (n*size_1k);
        if(unprot_size > 32*size_1k)
        {
            break;
        }
...
static void _bp_init_256m(enum HAL_FLASH_ID_T id, uint32_t total_size)
{
    ...
    while(1)
    {
        ...
        bp_map[ind].bp = 0x0000 | (0x40 + ((i+1)<<2)); // max bp = 0xe
        bp_map[ind].start_addr = 0x0;
        bp_map[ind].end_addr = prot_size - 1;
        ind ++;
        i ++;
...
static osMailQId _norflash_api_mailbox = NULL;
static uint8_t _norflash_api_mailbox_cnt = 0;

osThreadDef(_norflash_api_thread, osPriorityLow, 1, 4096, "norflash_api_thread");

osMailQDef (_norflash_api_mailbox, NORFLASH_API_MAILBOX_MAX, NORFLASH_API_MESSAGE_T);

static int _norflash_api_mailbox_init(void)
{
    // NORFLASH_API_TRACE(1, "%s,%d",__func__,__LINE__);
    _norflash_api_mailbox = osMailCreate(osMailQ(_norflash_api_mailbox), NULL);
    if (_norflash_api_mailbox == NULL)  {
...
static int _norflash_api_mailbox_put(NORFLASH_API_MESSAGE_T* msg_src)
{
    ...
    msg_p = (NORFLASH_API_MESSAGE_T*)osMailAlloc(_norflash_api_mailbox, 0);
    ASSERT(msg_p, "osMailAlloc error");
    msg_p->id = msg_src->id;
    msg_p->param = msg_src->param;

    status = osMailPut(_norflash_api_mailbox, msg_p);
    if (osOK == status)
    {
...
static int _norflash_api_mailbox_free(NORFLASH_API_MESSAGE_T* msg_p)
{
    ...
    status = osMailFree(_norflash_api_mailbox, msg_p);
    if (osOK == status)
    {
        _norflash_api_mailbox_cnt--;
    }
...
static int _norflash_api_mailbox_get(NORFLASH_API_MESSAGE_T **msg_p)
{
    ...
    evt = osMailGet(_norflash_api_mailbox, osWaitForever);
    if (evt.status == osEventMail) {
        *msg_p = (NORFLASH_API_MESSAGE_T*)evt.value.p;
        return 0;
    }
    return -1;
}
...
static int norflash_api_thread_init(void)
{
    ...
    tid = osThreadCreate(osThread(_norflash_api_thread), NULL);
    if (tid == NULL) {
...
#else
#if defined(FLASH_SUSPEND)
    hal_sleep_set_sleep_hook(HAL_SLEEP_HOOK_NORFLASH_API, norflash_api_flush_in_task);
#else //FLASH_SUSPEND
    hal_sleep_set_deep_sleep_hook(HAL_DEEP_SLEEP_HOOK_NORFLASH_API, norflash_api_flush_in_task);
#endif // FLASH_SUSPEND
#endif
                ...
                )
{
    ...
    if(!IS_ALIGN(start_addr,mod_info->mod_sector_len))
    {
