...
typedef enum  {
  ...
  osPriorityHigh          = SCHED_PRIORITY_MAX-3,          ///< priority: high
...
typedef struct os_timer_def  {
  ...
  osTimerAttr_t             *timer;    ///< pointer to internal data
} osTimerDef_t;


typedef struct os_timer_def_ex  {
  osTimerDef_t       os_timer_def;
  osTimerAttr_t      os_timer_cb;
} osTimerDefEx_t;
...
((osTimerDefEx_t*)&pTimer)->os_timer_def.timer->name = NULL; \
((osTimerDefEx_t*)&pTimer)->os_timer_def.timer->type = osTimerOnce; \
((osTimerDefEx_t*)&pTimer)->os_timer_def.timer->timerid = (timer_t)0; \
((osTimerDefEx_t*)&pTimer)->os_timer_def.timer->argument = 0; \
...
typedef struct os_mutex_def  {
  pthread_mutex_t mutex;    ///< pointer to internal data
} osMutexDef_t;
...
#define osMutexInit(pMutex)   \
 memset(&(((osMutexDefEx_t*)&pMutex)->os_mutex_def.mutex),0,sizeof(pthread_mutex_t))


/// Semaphore Definition structure contains setup information for a semaphore.
/// \note CAN BE CHANGED: \b os_semaphore_def is implementation specific in every CMSIS-RTOS.
typedef struct os_semaphore_def  {
  sem_t semaphore;    ///< pointer to internal data
} osSemaphoreDef_t;
...
{ 0,priority,stacksz,0,0,0,0,(pthread_func_t)name,0,task_name}
#endif
...
#else                            // define the object
#define osTimerDef(name, function)  \
osTimerAttr_t os_timer_attr_##name = {# name, osTimerOnce ,0, 0}; \
const osTimerDef_t os_timer_def_##name = \
{ (function), (&os_timer_attr_##name) }
#endif
