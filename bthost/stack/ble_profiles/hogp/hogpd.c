...
const gatt_att16_desc_t hogpd_att_db[HOGPD_IDX_NB] =
{
    ...
    [HOGPD_IDX_PROTO_MODE_VAL]               = { GATT_CHAR_PROTOCOL_MODE,        PROP(RD) |PROP(WR)| PROP(WC),              OPT(NO_OFFSET) | sizeof(uint8_t)           },
                                    ...
                                    uint16_t max_length)
{
    ...
    do
    {
        ...
        if(p_hogpd_env == NULL)
        {
            TRACE(0, "hogpd att read get: null hogpd env");
            break;
        }
        ...
        if (status != GAP_ERR_NO_ERROR)
        {
            TRACE(0, "hogpd att read get: conidx %d att idx not found %d hdl %d", conidx, status, hdl);
            break;
        }

        TRACE(0, "hogpd att read get: conidx %d hdl %d att_idx %d report_idx %d", conidx, hdl, att_idx, report_idx);
        ...
        if((status != GAP_ERR_NO_ERROR) || (!confirm_send))
        {
            TRACE(0, "hogpd att read get: status %d configm_send %d", status, confirm_send);
            break;
        }
                                   ...
                                   co_buf_t* p_data)
{
    ...
    do
    {
        ...
        if((p_hogpd_env == NULL) || (offset != 0))
        {
            TRACE(0, "hogpd att val set: reject due to hogpd env %p offset %d", p_hogpd_env, offset);
            break; // reject write if offset != 0
        }

        status = hogpd_get_att_idx(p_hogpd_env, hdl, &hid_idx, &att_idx, &report_idx);
        if (status != GAP_ERR_NO_ERROR)
        {
            TRACE(0, "hogpd att val set: conidx %d att idx not found %d hdl %d", conidx, status, hdl);
            break;
        }

        TRACE(0, "hogpd att val set: conidx %d hdl %d att_idx %d report_idx %d", conidx, hdl, att_idx, report_idx);
...
uint16_t hogpd_report_read_cfm(uint8_t conidx, uint16_t token, uint16_t status, uint16_t tot_length, co_buf_t* p_buf)
{
    ...
    if(p_hogpd_env != NULL)
    {
        LOG_I("p_hogpd_env->user_lid:%x,token:%x",p_hogpd_env->user_lid,token);
    ...
    LOG_I("%s status:%x",__func__,status);
    return (status);
}

uint16_t hogpd_report_write_cfm(uint8_t conidx, uint16_t token, uint16_t status)
{
    LOG_I("%s token:%x status:%x",__func__,token,status);
    hogpd_env_t* p_hogpd_env = PRF_ENV_GET(HOGPD, hogpd);
...
uint16_t hogpd_report_upd(uint8_t conidx, uint8_t hid_idx, uint8_t report_idx, uint8_t report_type, co_buf_t* p_buf)
{
    ...
    hogpd_env_t* p_hogpd_env = PRF_ENV_GET(HOGPD, hogpd);
    LOG_I("%s report_type:%x",__func__,report_type);
    if(p_buf == NULL)
    {
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        TRACE(0, "hogpd report upd: status %d", status);
    }
                          ...
                          struct hogpd_db_cfg *p_params, const hogpd_cb_t* p_cb)
{
    ...
    //struct hogp_report_ref report_ref;
    ...
    do
    {
        ...
        if(status != GAP_ERR_NO_ERROR)
        {
            TRACE(0, "hogpd init: srv register failed %d", status);
            break;
        }

        //-------------------- allocate memory required for the profile  ---------------------
        p_hogpd_env = (hogpd_env_t *) ke_malloc(sizeof(hogpd_env_t), KE_MEM_PROFILE);
        ...
        for (svc_idx = 0; ((svc_idx < p_params->hids_nb) && (status == GAP_ERR_NO_ERROR)); svc_idx++)
        {
            ...
            p_hids_db[svc_idx] = (gatt_att16_desc_t *) ke_malloc(HOGPD_ATT_MAX * sizeof(gatt_att16_desc_t), KE_MEM_NON_RETENTION);
        ...
        if (status != GAP_ERR_NO_ERROR)
        {
            TRACE(0, "hogpd init: range reserve failed %d", status);
            break;
        }
        ...
        for (svc_idx = 0; ((svc_idx < p_params->hids_nb) && (status == GAP_ERR_NO_ERROR)); svc_idx++)
        {
            ...
            if(status != GAP_ERR_NO_ERROR)
            {
                TRACE(0, "hogpd init: svc16 add failed %d", status);
                break;
            }
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        TRACE(0, "hogpd init: status failed %d", status);
    }
