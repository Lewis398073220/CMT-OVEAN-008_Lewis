...
uint16_t blpc_enable(uint8_t conidx, uint8_t con_type, const bps_content_t* p_bps)
{
    ...
    if(p_blpc_env != NULL)
    {
        if ((conidx < BLE_CONNECTION_MAX) && (p_blpc_env->p_env[conidx] == NULL))
        {
            ...
            p_blpc_env->p_env[conidx] = (struct blpc_cnx_env *) ke_malloc(sizeof(struct blpc_cnx_env), KE_MEM_PROFILE);
                          ...
                          const void* p_params, const blpc_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_blpc_env = (blpc_env_t*) ke_malloc(sizeof(blpc_env_t), KE_MEM_PROFILE);
