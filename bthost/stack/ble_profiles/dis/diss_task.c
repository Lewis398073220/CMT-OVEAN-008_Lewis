    ...
    if (param->value < DIS_CHAR_MAX)
    {
        ...
        if (handle != GATT_INVALID_HDL)
        {
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                // allocate value data
                val = (struct diss_val_elmt *) ke_malloc(sizeof(struct diss_val_elmt) + param->length, KE_MEM_PROFILE);
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if ((diss_env->req_val == param->value) && (param->length))
    {
        prf_buf_alloc(&p_data, param->length);
        memcpy(co_buf_data(p_data), param->data, param->length);

        gatt_srv_att_read_get_cfm(diss_env->req_conidx, diss_env->req_user_lid,
            diss_env->req_token, GAP_ERR_NO_ERROR, param->length, p_data);
        TRACE(1, "%s value:%d, dataLen:%d", __func__, param->value, param->length);
        // Release the buffer
        if(p_data != NULL)
        {
    ...
    {
        gatt_srv_att_read_get_cfm(diss_env->req_conidx, diss_env->req_user_lid,
                                  diss_env->req_token, ATT_ERR_APP_ERROR, 0, NULL);
    }
...
void diss_task_init(struct ke_task_desc *task_desc,void *p_env)
{
    ...
    diss_env_t *diss_env = (diss_env_t *)p_env;
