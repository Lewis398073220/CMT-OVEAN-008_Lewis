...
#ifdef SPOT_ENABLED
__STATIC int send_beacon_data_via_notification_handler(ke_msg_id_t const msgid,
                                struct gfpsp_send_data_req_t const *param,
                                ke_task_id_t const dest_id,
                                ke_task_id_t const src_id)
{
    send_notifiction(param->connecionIndex, GFPSP_IDX_BEACON_ACTIONS_VAL, param->value, param->length);
    return (KE_MSG_CONSUMED);
}
#endif
...
#ifdef SPOT_ENABLED
    {GFPSP_BEACON_ACTIONS_WRITE_NOTIFY,      (ke_msg_func_t)send_beacon_data_via_notification_handler},
#endif
};
