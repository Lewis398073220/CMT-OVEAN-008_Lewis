                      ...
                      const svc_content_t* p_txps)
{
    ...
    if(p_proxm_env != NULL)
    {
        if ((conidx < BLE_CONNECTION_MAX) && (p_proxm_env->p_env[conidx] == NULL))
        {
            ...
            p_proxm_env->p_env[conidx] = (struct proxm_cnx_env *) ke_malloc(sizeof(struct proxm_cnx_env), KE_MEM_PROFILE);
                          ...
                          const void* p_params, const proxm_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_proxm_env = (proxm_env_t*) ke_malloc(sizeof(proxm_env_t), KE_MEM_PROFILE);
