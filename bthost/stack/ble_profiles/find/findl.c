...
uint16_t findl_enable(uint8_t conidx, uint8_t con_type, const ias_content_t* p_ias)
{
    ...
    if(p_findl_env != NULL)
    {
        if ((conidx < BLE_CONNECTION_MAX) && (p_findl_env->p_env[conidx] == NULL))
        {
            ...
            p_findl_env->p_env[conidx] = (struct findl_cnx_env *) ke_malloc(sizeof(struct findl_cnx_env), KE_MEM_PROFILE);
                          ...
                          const void* p_params, const findl_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_findl_env = (findl_env_t*) ke_malloc(sizeof(findl_env_t), KE_MEM_PROFILE);
