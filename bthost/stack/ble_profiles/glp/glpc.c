...
uint16_t glpc_enable(uint8_t conidx, uint8_t con_type, const gls_content_t* p_gls)
{
    ...
    if(p_glpc_env != NULL)
    {
        if ((conidx < BLE_CONNECTION_MAX) && (p_glpc_env->p_env[conidx] == NULL))
        {
            ...
            p_glpc_env->p_env[conidx] = (struct glpc_cnx_env *) ke_malloc(sizeof(struct glpc_cnx_env), KE_MEM_PROFILE);
                          ...
                          const void* p_params, const glpc_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_glpc_env = (glpc_env_t*) ke_malloc(sizeof(glpc_env_t), KE_MEM_PROFILE);
