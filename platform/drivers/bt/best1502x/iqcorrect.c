...
static int bt_tx_pa_calib_t0_energy_get(void)
{
    ...
    //     }
    //     caculate_energy_main_test(st,&tmp_energy,&tmp_energy1,fftsize);
    //     BT_DRV_TRACE(1,"tmp_energy: %d ",tmp_energy1);
...
#ifdef PA_BIAS_DEPUG
        TRACE(0,"0xca:0x%x, energy:%d", pa_calib_rf_0xca[ii], tmp_energy1);
#endif
    }
    ...
    TRACE(0,"%s, min_energy:0x%x, ca:0x%x", __func__, min_energy, pa_calib_rf_0xca[min_num]);
    return min_energy;
}
...
static void bt_tx_pa_calib_t2(void)
{
    ...
    btdrv_write_rf_reg(0x10, 0x3);      //enable 0xCA reg
    //add by xulicheng
    for (int k = 0; k < TX_PULLING_CAL_CNT; k++) {
        int tmp_energy, tmp_energy1, min_energy = 1000000000;

        rf_local = 0x9640 + k * 0xA0;
        dig2     = 0x800A0003 + k * 0xA;

        if(k<=2) {
            dig1 = 0x80bb80d6 + k*0x10BB6;
        } else if((k>2) && (k<5)) {
            dig1 = 0x80bea3f9 + (k-3)*0x10BB6;
        } else {
            dig1 = 0x80c0bb66 + (k-5)*0x10BB6;
        }
...
//         }
        ...
        fdata1_bank2_addr = 0x174 + k;

        BTDIGITAL_REG_WR(0xd0220c00, 0x0);      //turn of tx on

        BTRF_REG_SET_FIELD(fdata1_bank2_addr, 0xFF, 0, pa_calib_rf_0xca[min_num]);
    }
    btdrv_write_rf_reg(0x10, 0x0);      //restore 0xCA reg
}
...
void bt_tx_pulling_calib(void)
{
    ...
    BTRF_REG_SET_FIELD(0x24, 0x7, 5, 0x4);      //dr tx gain idx5
    ...
    BTRF_REG_SET_FIELD(0x24, 0x7, 5, 0x3);      //dr tx gain idx4
    bt_tx_pulling_algorithm(calib_tx_idx);
